...
[r,c]= size(posisi);
    ...
    mobilbaru(:,posisi(1,:))=[];
    %end
matriksjarakkota2 = mobilbaru;
save matriksjarakkota2
if c>=8
...
%--------------------------------------------------------
n=1;
[sa,si]=size(matriksjarakkota2);
%si=17;

%Step-1: Dilakukan perhitungan pergerakan pertama

A=matriksjarakkota2(n,:);
x=min(A(A~=0));
posisi=find(A==x);
 
%kolom 1 disimpan dulu
start=matriksjarakkota2(:,1);
 
%backup untuk data matrik asli
initial=matriksjarakkota2;
...
load ('matriksjarakkota2.mat');
%load ('kota.mat');
%load ('akhir.mat');
%load ('matriksjarak.mat');
[jin,jun]=size(matriksjarakkota2);
mobil2= matriksjarakkota2((2:jin),(2:jun));
kota = matriksjarakkota2(1,(2:jun))';
akhir = matriksjarakkota2((2:jin),1);
[r,c] = size(mobil2);
nk=c; %jumlah kota
np=20;
pa=0.25;
